    ...
    <div className="flex gap-3  rounded-xl mt-3">
      ...
      <span className="line-clamp-1">{address}</span>
        ...
        <div className="flex items-center text-xs mt-1">
          <Star size={14} className="text-green-600 mr-1" weight='fill' color='green'  />
          {rating}
        </div>
        <div className="text-base font-bold mt-1">₹{price} <span className="text-sm font-normal text-gray-500">for 1 night</span></div>
      </div>
    </div>
  )
}
